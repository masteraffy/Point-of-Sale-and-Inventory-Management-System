    ...
    public partial class Pos_Checkout : Form
    {
                        ...
                        if (errorCount == 0)
                        {
                            ...
                            if (data.rowAffected > 0)
                            {
                                if (cmbReceipt.SelectedIndex == 0)
                                {
                                    ...
                                    cn.Open();
                                    cm = new MySqlCommand("Insert into tblLogs (datelog, timelog, full_name, action, module)" +
                                        "Values('" + date.ToString("yyyy-MM-dd") + "', '" + date.ToString("hh:mm:tt") + "', '" + username + "', 'Checked Out Transaction Number: " +
                                        transnumber.ToString() + " by: " + username + "', 'POS')", cn);
                                    cm.ExecuteNonQuery();
                                    cn.Close();
                                    prf.ShowDialog();
                                    this.Close();
        ...
        public void search()
        {
            try
            {
                ...
                cn.Open();
                string sql = "SELECT * FROM tblposcustomerinfo";
                cm = new MySqlCommand(sql, cn);
                MySqlDataReader sdr = null;
                sdr = cm.ExecuteReader();
                while(sdr.Read())
                {
                    col.Add(sdr["FullName"].ToString());
                }
                sdr.Close();
        ...
        public void Validate1()
        {
            ...
            if (string.IsNullOrEmpty(txtAmount.Text) || txtAmount.Text == "0.00")
            {
